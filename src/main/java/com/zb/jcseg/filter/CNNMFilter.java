...
public class CNNMFilter {
     ...
     * i have put the chars into the lexicon file lex-cn-numeric.lex for the old version. <r /> it's better to follow
     * the current work.
     */
    ...
    public static int cnNumericToArabic(String cnn, boolean flag) {
        ...
        if (flag) cnn = cnn.replace('佰', '百').replace('仟', '千').replace('拾', '十').replace('零', ' ');
        ...
        if (yi > -1) {
            ...
            if (yi < cnn.length() - 1) cnn = cnn.substring(yi + 1, cnn.length());
            else cnn = "";

            if (cnn.length() == 1) {
                int arbic = isCNNumeric(cnn.charAt(0));
                if (arbic <= 10) val += arbic * 10000000;
                cnn = "";
            }
        }
        ...
        if (wan > -1) {
            ...
            if (wan < cnn.length() - 1) cnn = cnn.substring(wan + 1, cnn.length());
            else cnn = "";
            if (cnn.length() == 1) {
                int arbic = isCNNumeric(cnn.charAt(0));
                if (arbic <= 10) val += arbic * 1000;
                cnn = "";
            }
        }
        ...
        if (qian > -1) {
            ...
            if (qian < cnn.length() - 1) cnn = cnn.substring(qian + 1, cnn.length());
            else cnn = "";
            if (cnn.length() == 1) {
                int arbic = isCNNumeric(cnn.charAt(0));
                if (arbic <= 10) val += arbic * 100;
                cnn = "";
            }
        }
        ...
        if (bai > -1) {
            ...
            if (bai < cnn.length() - 1) cnn = cnn.substring(bai + 1, cnn.length());
            else cnn = "";
            if (cnn.length() == 1) {
                int arbic = isCNNumeric(cnn.charAt(0));
                if (arbic <= 10) val += arbic * 10;
                cnn = "";
            }
        }
        ...
        if (shi > -1) {
            if (shi == 0) val += 1 * 10;
            else val += cnNumericToArabic(cnn.substring(0, shi), false) * 10;
            if (shi < cnn.length() - 1) cnn = cnn.substring(shi + 1, cnn.length());
            else cnn = "";
        }
    ...
    /*
     * public static void main(String[] args) { ADictionary.isCNNumeric('一'); int val = 0; long s = System.nanoTime();
     * //val = cnNumericToArabic("三亿二千零六万七千五百六", true); //val = cnNumericToArabic("一九九八", true); long e =
     * System.nanoTime(); System.out.format("Done["+val+"], cost: %.5fsec\n", ((float)(e - s)) / 1E9); }
     */
}
