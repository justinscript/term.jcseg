...
public class ENSCFilter {
     ...
     * private static Map<Character, Character> enKeepChar = null; static { enKeepChar = new HashMap<Character,
     * Character>( ( int )(EN_KEEP_CHARS.length / 1.7) + 1, 0.85f ); //set the keep char's keep status for ( int j = 0;
     * j < EN_KEEP_CHARS.length; j++ ) enKeepChar.put(EN_KEEP_CHARS[j], EN_KEEP_CHARS[j]); }
     */
    ...
    /**
     * a static method to replace the full-width char to the half-width char in a given string. (65281-65374 for
     * full-width char) <br />
     * 
     * @param str
