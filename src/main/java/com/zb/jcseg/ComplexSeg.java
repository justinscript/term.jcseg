...
public class ComplexSeg extends ASegment {
    ...
    public ComplexSeg(JcsegTaskConfig config, JcsegDictionary dic) throws IOException {
        super(config, dic);
    }

    public ComplexSeg(Reader input, JcsegTaskConfig config, JcsegDictionary dic) throws IOException {
    ...
    public IChunk getBestCJKChunk(char chars[], int index) {
        ...
        if (mwords.length == 1 && mwords[0].getType() == ILexicon.UNMATCH_CJK_WORD) {
        ...
        for (int x = 0; x < mwords.length; x++) {
            ...
            if (idx_2 < chars.length) {
                ...
                /*
                 * the first try for the second layer returned a UNMATCH_CJK_WORD here, just return the largest length
                 * word in the first layer.
                 */
                if (mword2.length == 1 && mword2[0].getType() == ILexicon.UNMATCH_CJK_WORD) {
                ...
                for (int y = 0; y < mword2.length; y++) {
                    ...
                    if (idx_3 < chars.length) {
                        ...
                        for (int z = 0; z < mword3.length; z++) {
                            ...
                            wArr.add(mwords[x]);
                            wArr.add(mword2[y]);
                            if (mword3[z].getType() != ILexicon.UNMATCH_CJK_WORD) wArr.add(mword3[z]);
        ...
        if (chunkArr.size() == 1) return chunkArr.get(0);

        /*
         * Iterator<IChunk> it = chunkArr.iterator(); while ( it.hasNext() ) { System.out.println(it.next()); }
         * System.out.println("-+---------------------+-");
         */
        ...
        if (afterChunks.length >= 2) {
            ...
            if (afterChunks.length >= 2) {
                ...
                if (afterChunks.length >= 2) {
                    ...
                    if (afterChunks.length >= 2) {
                        /*
                         * Attention: there is chance for length of the chunks over 2 even after the four rules. we use
                         * the LASTRule to clear the Ambiguity.
                         */
