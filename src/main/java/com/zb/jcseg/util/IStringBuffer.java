...
public class IStringBuffer {
    ...
    public void append(String str) {
        ...
        if (count + str.length() > buff.length) resizeTo((count + str.length()) * 2 + 1);
    ...
    public void append(char[] chars, int start, int length) {
        ...
        if (start + length >= chars.length) throw new IndexOutOfBoundsException();

        // check the necessary to resize the buffer.
        if (count + length > buff.length) resizeTo((count + length) * 2 + 1);
    ...
    public char charAt(int idx) {
        ...
        if (idx >= count) throw new IndexOutOfBoundsException("idx >= buffer.length");
        return buff[idx];
    }
