...
/**
 * Separate Chaining table - implementation the hash table the elements match is based on the equals method of the
 * Object (so better to rewrite the equals method for the complex Object)
 * 
...
public class JHashMap {
    ...
    public boolean containsValue(IWord val) {
        ...
        for (int i = 0; i < table.length; i++) {
            for (e = table[i]; e != null; e = e._next) {
                if (val.equals(e.val) || val == e.val) return true;
            }
        }
        return false;
    }
     ...
     * 
     * @param key
     * @return if there is a mapping for the key return the value of the Entry or return null
     */
    ...
    /**
     * Returns the entry associated with the specified key in the hash table.
     * 
     * @returns null if the HashMap contains no mapping for the key. else the associated Entry
     */
    final Entry getEntry(String key) {
        int hash = (key == null) ? 0 : hash(key);
        String k;
        for (Entry e = table[hash]; e != null; e = e._next) {
    ...
    /**
     * Associates the specified value with the specified key in the table if the map contains the mapping for the key,
    ...
    void addEntry(String key, IWord val, int hash) {
        ...
        /*
         * Attention: do not use the following condition check it will cause memory use up error. here use threshold
         * instead
         */
     ...
     * @return if there is a mapping for the key return the old value else return null
     */
    ...
    /**
     * if table.length times fillFactor is larger than the size, we need to reload the hash table
     */
    private void reHash() {
        Entry[] _src = table;
    ...
    public int hash(String key) {
        ...
        if (hashVal < 0) hashVal = hashVal + size;
